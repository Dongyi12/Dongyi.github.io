...
class MySketch:
    ...
    def setup(self):
        ...
        # Calculate the number of samples in a bar and a beat
        samples_in_bar = len(audio) // 2
        samples_in_beat = samples_in_bar // 4

# Cut the first two beats from audio2
        cut = audio2[:samples_in_beat * 2]

# Overwrite the second and third beats in audio with the cut from audio2
        audio[samples_in_beat:samples_in_beat * 3] = cut

# Save the modified audio
        sf.write("week8\\images\\audio_drums.wav", audio, sr)

        print(f"The modified audio has been saved as 'modified_audio.wav'.")
        ...
        cut = audio2[:samples_in_beat*2 ]
        audio[samples_in_beat:samples_in_beat*3] = cut
        dot.music.start_sample_stream(audio3, sr=sr)
        self.show_beat = 0 
